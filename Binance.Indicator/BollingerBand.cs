    ...
    public class BollingerBand : IndicatorCalculatorBase<BollingerBandSerie>
    {
        ...
        public override BollingerBandSerie Calculate()
        {
            ...
            for (int i = 0; i < OhlcList.Count; i++)
            {
                ...
                totalAverage += (double)OhlcList[i].Close;
                totalSquares += Math.Pow((double)OhlcList[i].Close, 2);
                ...
                if (i >= Period - 1)
                {
                    ...
                    //double bPercent = ((double)OhlcList[i].Close - down) / (up - down);
                    //bollingerBandSerie.BPercent.Add(bPercent);

                    totalAverage -= (double)OhlcList[i - Period + 1].Close;
                    totalSquares -= Math.Pow((double)OhlcList[i - Period + 1].Close, 2);
                }
                else
                {
